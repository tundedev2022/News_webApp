    ...
    const handleSubmit = (e)=>{
      ...
      if(!title || !content){
        if(!title)setTitleValid(false)
          if(!content)setContentValid(true)
            return
      }
      const newBlog = {
        image : image || letterImg,
        title,
        content,
      }
        ...
        {submitted && <p className='submission-message'> post submitted!</p>}
            ...
            <form onSubmit={handleSubmit} >
              <div className="img-upload">
                ...
                <input
                 type="text" placeholder='Add Title (Max 60 Characters)'
                ...
                value={content} onChange={handleContentChange}></textarea>
                <button type='submit' className="submit-btn">{isEditing ? 'Update Post' : 'Submit Post'}
                  </button> 
            </form>
          </div>


        <button className="blogs-close-btn" onClick={onBack}>Back <i className="bx bx-chevron-right"></i>
        </button>
      </div>
